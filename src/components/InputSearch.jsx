...
export default function InputSearch() {
	...
	const [prefixURL, setPrefixURL] = useState(
		searchMethodOptions[searchMethodIndex].url,
	);
	...
	useEffect(() => {
		localStorage.setItem(
			"SearchMethodIndex",
			JSON.stringify(searchMethodIndex),
		);
	...
	return (
		...
		>
			<a hidden ref={linkRef} href={`${prefixURL + inputValue}`}></a>
			<div className="flex flex-row border-2 border-[#585b70] w-full px-2 rounded-l-xl cursor-text">
			...
			>
				<i
					className={`bi bi-${commandName === "Search" && searchMethodOptions[searchMethodIndex].icon} leading-none`}
				></i>
			</button>
		</form>
	);
